...
- 2026-08-17: 프로필 편집기의 단축키 세트가 저장 데이터에서 빠져 재실행 시 초기화되던 문제를 수정했다. 최대 3개 세트를 프로필에 보존하고, 앱 시작과 프로필 저장·수정·삭제 때 Windows 전역 단축키 등록을 다시 구성한다. `pywin32`의 성공 반환값과 Qt 네이티브 `WM_HOTKEY` 수신도 바로잡았다. 전용 5건, 전체 52건 테스트, 실제 키 입력 재현, 공식 EXE 재빌드가 통과했다. 최신 EXE를 `C:\app\WindowResizer.exe`에 교체했고 해시 일치를 확인했다. 실제 프로필 단축키 UI 확인만 남았다.
