...
class TrayStatusDialog(QDialog):
    ...
    def update_status(self):
        ...
        try:
            ...
            status_text = f"""창모드 리사이저 0.01.2 상태
{'=' * 50}
...
class SystemTrayManager:
    ...
    def initialize(self, app: QApplication) -> bool:
        ...
        self.tray_icon.setToolTip("창모드 리사이저 0.01.2")
        ...
        try:
            ...
            if stats['is_running']:
                if stats['errors'] > stats.get('last_error_count', 0):
                    ...
                    self.tray_icon.setToolTip("창모드 리사이저 0.01.2 (오류)")
                else:
                    self.tray_icon.setIcon(self.create_tray_icon("active"))
                    self.tray_icon.setToolTip("창모드 리사이저 0.01.2 (작동 중)")
                
                self.start_action.setEnabled(False)
                self.stop_action.setEnabled(True)
            else:
                self.tray_icon.setIcon(self.create_tray_icon("idle"))
                self.tray_icon.setToolTip("창모드 리사이저 0.01.2 (중지됨)")
